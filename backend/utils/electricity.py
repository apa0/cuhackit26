...
def predict_electricity_impact(
    ...
    weight = calculate_pressure_weight(
        county_dc,
        adjacent_dc_total,
        alpha,
        beta
    ...
    # Convert annual baseline growth to monthly compounding
    ...
    for month in range(1, months_to_project + 1):
        ...
        projected_bill = round(
            farm_monthly_cost * growth_factor * weight,
            2
        )
    ...
    pct_increase = round(
        (total_increase / farm_monthly_cost) * 100,
        2
    ) if farm_monthly_cost else 0

    plain = (
        f"{county} County has {county_dc} data center(s). "
        f"Adjacent counties contribute {adjacent_dc_total} additional center(s). "
        f"Applying structural grid pressure and baseline growth, "
        f"your monthly bill is projected to increase from "
        f"${farm_monthly_cost:,.2f} to "
        f"${projections[-1]['projected_monthly_bill_usd']:,.2f} "
        f"over {months_to_project} months "
...
def lambda_handler(event, context):
    ...
    try:
        result = predict_electricity_impact(
            ...
            alpha=float(event.get("alpha", 0.015)),
            beta=float(event.get("beta", 0.35)),
        )

        s3_uri = save_projection_to_s3(
            result,
            key=f"projections/{event['county'].lower()}_projection.json"
        )
